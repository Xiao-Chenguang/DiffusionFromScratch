...
dataset_rsz = CelebA(
    "/home/binxuwang/Datasets", target_type=["attr"], transform=tfm, download=False
)  # ,"identity"
# %%
...
nantoken = 40
maxlen = int((y_col.sum(dim=1)).max().item())
yseq_data = torch.ones(y_col.size(0), maxlen, dtype=torch.int).fill_(nantoken)
